<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center">
            <a href="{{ route('masyarakat.pengaduan.index') }}" class="mr-4 text-gray-400 hover:text-green-600 transition">
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Alert Gagal --}}
            @if(session('error'))
                <div class="mb-6 bg-red-50 border border-red-100 text-red-700 p-4 rounded-2xl shadow-sm flex items-center" role="alert">
            ...
            <div class="bg-white p-8 md:p-10 shadow-sm rounded-3xl border border-gray-100">
                <form action="{{ route('masyarakat.pengaduan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        <label class="block font-black text-green-800 uppercase tracking-wider text-xs mb-2">Judul Aduan</label>
                    ...
                    <div>
                        <label class="block font-black text-green-800 uppercase tracking-wider text-xs mb-2">Deskripsi Laporan</label>
                        <textarea name="deskripsi" rows="5" 
                            class="w-full border-gray-200 rounded-xl shadow-sm focus:border-green-500 focus:ring focus:ring-green-200 focus:ring-opacity-50 transition duration-200" 
                            placeholder="Ceritakan detail kejadian secara lengkap..." maxlength="100" required>{{ old('deskripsi') }}</textarea>
                    </div>

                    <div class="p-6 bg-green-50 rounded-2xl border border-green-100">
                        <label class="block font-black text-green-700 uppercase tracking-wider text-xs mb-2">Upload Dokumentasi (Foto)</label>
                    ...
                    <div class="flex justify-end items-center gap-6 pt-4">
                        <a href="{{ route('masyarakat.pengaduan.index') }}" class="text-gray-400 font-bold hover:text-red-500 transition">
                            Batal
                        </a>
                        <button type="submit" class="bg-green-600 text-white px-10 py-3 rounded-xl font-bold hover:bg-green-700 transition duration-300 shadow-lg shadow-green-100 transform hover:-translate-y-1">
                            Kirim Aduan
                        </button>
                    </div>
                </form>
